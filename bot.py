...
ADMIN_ID = 764559466  # استبدل بمعرف المستخدم الخاص بك
...
DEFAULT_CONFIG = {
    "groups": {},  # {"group_id": {"totp_secret": "SECRET", "interval": 600, "message_style": 1, "timezone": "UTC"}}
    "admins": [ADMIN_ID]
}
...
async def send_auth_message(bot, group_id):
    ...
    if message_style == 1:
        ...
        message += f"⏱️ الرمز التالي خلال: {interval_text}\n"
        message += f"⏭️ الرمز التالي في: {next_time}"
    else: # message_style == 3
        message += f"⏱️ الرمز التالي خلال: {interval_text}\n"
        message += f"⏰ الوقت الحالي: {current_time}\n"
        message += f"⏭️ الرمز التالي في: {next_time}"
    
    keyboard = [[InlineKeyboardButton("📋 نسخ الرمز", callback_data=f"copy_code_{group_id}")]]
    reply_markup = InlineKeyboardMarkup(keyboard)
...
async def button_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """معالجة النقر على الأزرار (خارج المحادثة)"""
    query = update.callback_query
    await query.answer() # الاستجابة السريعة لتأكيد استلام الضغطة
