...
reset_task = None  # لتخزين مهمة إعادة تعيين المحاولات اليومية
...
async def reset_daily_attempts_async():
    """إعادة تعيين عدد المحاولات اليومية للمستخدمين عند منتصف الليل (نسخة غير متزامنة)"""
    global users
    ...
    while True:
        ...
        save_users()
        logger.info("تم إعادة تعيين عدد المحاولات اليومية للمستخدمين")
...
async def main():
    ...
    # بدء مهمة إعادة تعيين المحاولات اليومية
    asyncio.create_task(reset_daily_attempts_async())
    
    # بدء البوت باستخدام run_polling (الطريقة الحديثة)
